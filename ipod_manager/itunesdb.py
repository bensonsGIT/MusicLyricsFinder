...
class iTunesDB:
    ...
    def _parse_mhit(self, data: bytes, pos: int) -> tuple["Track", int]:
        ...
        t.bitrate = _u32(36)
        t.duration_ms = _u32(40)
        t.file_size = _u32(44)
        t.time_added = _to_unix(_u32(52))
        t.play_count = _u32(56)
        t.last_played = _to_unix(_u32(64))
        t.disc_number = _u32(68)
        t.disc_count = _u32(72)
        t.track_number = _u32(76)
        t.track_count = _u32(80)
        t.year = _u32(84)
        t.bpm = _u32(88)
        t.dbid = _u64(104)
    ...
    def _parse_mhod_string(self, data: bytes, pos: int) -> str:
        ...
        return raw.decode("utf-8" if encoding == 1 else "utf-16-le", errors="replace").replace("\x00", "")
    ...
    def _build_mhit(self, t: Track) -> bytes:
        ...
        struct.pack_into("<B", hdr, 30, t.rating)
        struct.pack_into("<I", hdr, 32, t.sample_rate)
        struct.pack_into("<I", hdr, 36, t.bitrate)
        struct.pack_into("<I", hdr, 40, t.duration_ms)
        struct.pack_into("<I", hdr, 44, t.file_size)
        struct.pack_into("<I", hdr, 48, _to_mac(t.time_added) if t.time_added else _to_mac())
        struct.pack_into("<I", hdr, 52, _to_mac(t.time_added) if t.time_added else _to_mac())
        struct.pack_into("<I", hdr, 56, t.play_count)
        struct.pack_into("<I", hdr, 60, t.play_count)
        struct.pack_into("<I", hdr, 64, _to_mac(t.last_played) if t.last_played else 0)
        struct.pack_into("<I", hdr, 68, t.disc_number)
        struct.pack_into("<I", hdr, 72, t.disc_count)
        struct.pack_into("<I", hdr, 76, t.track_number)
        struct.pack_into("<I", hdr, 80, t.track_count)
        struct.pack_into("<I", hdr, 84, t.year)
